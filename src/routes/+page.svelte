<script lang="ts">
	...
	creds = '0';
	if (browser) {
		const dummy_credit = localStorage.getItem('dobber_id');
		if (typeof dummy_credit === 'string' || typeof dummy_credit === 'number') {
			creds = dummy_credit
		}
	}
	...
	async function onCreate() {
		...
		try {
			...
			if (data && data.image_b64 && !error) {
				...
				if (browser) {
					const credit = localStorage.getItem('dobber_id');
					if (typeof credit === 'string' || typeof credit === 'number') {
						let data = parseInt(credit) - 1;
						if (data < 0) data = 0;
						localStorage.setItem('dobber_id', data.toString());
						creds = data.toString();
					}
				}
