<script lang="ts">
	...
	import IconCredit from '$components/icons/Iconcredit.svelte';
...
<nav bind:clientHeight={$navbarHeight} class="w-full flex flex-col sticky -top-px z-50 transition">
	...
	<div class="w-full flex flex-row items-center justify-between relative z-0">
		...
		<div class="flex flex-1 flex-wrap items-center justify-end relative">
			<a
				class="border-1 px-3 py-1 mx-2 border-blue-50 cursor-pointer bg-c-bg shadow-xl shadow-c-shadow/[var(--o-shadow-strong)] 
			 rounded-2xl md:rounded-xl ring-2 ring-c-bg-secondary flex flex-row justify-center items-center hover:bg-c-bg-secondary transition"
			 href="/credit-recharge"
			>
				<IconCredit class="w-8 h-8 p-0 mt-2" />
				<div>{ credits }</div>
			</a>
